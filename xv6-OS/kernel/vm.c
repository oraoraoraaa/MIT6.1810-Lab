  ...
  for (i = 0; i < sz; i += PGSIZE)
    {
      ...
      // only COW-mark pages that are originally writtable
      if (*pte & PTE_W)
...
err:
  uvmunmap (new, 0, i / PGSIZE,
            0); // must set to 0, otherwise would clear parent's content
  return -1;
}
...
vmfault (pagetable_t pagetable, uint64 va, int read)
{
  ...
  if (pte != 0 && (*pte & PTE_V) && (*pte & PTE_COW) && !(*pte & PTE_W))
    ...
    {
      // is a lazy allocation page fault
      if (ismapped (pagetable, va))
        {
          return 0; // va is already mapped
        }
      mem = (uint64)kalloc ();
      if (mem == 0)
        return 0;
